...
func NatsCredentials(platformData *types.PlatformData) error {
	...
	systemManagerPub, systemManagerSeed, err := CreateUserNatsNkey()
	if err != nil {
		return err
	}
	platformData.Certs.NatsCerts.SystemManagerNKeyPublic = systemManagerPub
	platformData.Certs.NatsCerts.SystemManagerNKeySeed = systemManagerSeed
...
func GenerateHexKey(n int) string {
	...
	if _, err := rand.Read(b); err != nil {
		// crypto/rand failure is unrecoverable at platform creation time
		panic(fmt.Sprintf("utils.GenerateHexKey: crypto/rand failed: %v", err))
	}
