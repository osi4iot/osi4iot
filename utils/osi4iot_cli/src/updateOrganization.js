...
const removeMasterDevicesQuestions = (accessToken, osi4iotState, orgToUpdate, masterDevicesInOrg, orgData) => {
	const numMasterDevicesToRemove = masterDevicesInOrg.length - previousAnswers.NUMBER_OF_MASTER_DEVICES_IN_ORG;
	const table = new Table({
		head: [
			clc.cyanBright('Id'),
	...
	inquirer
		...
		.then(async (answers) => {
			...
			for (let irow = 0; irow < answers.MASTER_DEVICES_TO_REMOVE; irow++) {
				if (answers.MASTER_DEVICES_TO_REMOVE === "selected") {
...
const requestUpdateOrg = async (accessToken, osi4iotState, orgToUpdate, orgData) => {
	...
	if (currentNumMasterDevicesInOrg < orgData.NUMBER_OF_MASTER_DEVICES_IN_ORG) {
		for (let idev = currentNumMasterDevicesInOrg; idev < orgData.NUMBER_OF_MASTER_DEVICES_IN_ORG; idev++) {
			...
			osi4iotState.certs.mqtt_certs.organizations[orgIndex].master_devices.push(master_device);
		}
	}
	...
	if (currentNumMasterDevicesInOrg > orgData.NUMBER_OF_MASTER_DEVICES_IN_ORG) {
		...
		osi4iotState.certs.mqtt_certs.organizations[orgIndex].master_devices = master_devices;
	}
	...
	const updateOrgData = {
		...
		orgHash: org_hash,
		masterDeviceHashes: osi4iotState.certs.mqtt_certs.organizations[orgIndex].master_devices.map(md => md.md_hash),
	};
