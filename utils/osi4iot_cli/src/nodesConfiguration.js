...
const installDocker = async (nodeData, isLocalDeploy) => {
    ...
    if (!fs.existsSync('./installation_scripts/docker_install.sh')) {
        execSync("curl -o ./installation_scripts/docker_install.sh https://raw.githubusercontent.com/osi4iot/osi4iot/master/utils/osi4iot_cli/installation_scripts/docker_install.sh");
    }
    if (isLocalDeploy) {
        try {
        ...
        try {
            ...
            await execShellCommand(`ssh ${userName}@${nodeIP} bash docker_install.sh`)
...
const installUFW = async (nodeData) => {
    ...
    if (!fs.existsSync('./installation_scripts/ufw_install.sh')) {
        execSync("curl -o ./installation_scripts/ufw_install.sh https://raw.githubusercontent.com/osi4iot/osi4iot/master/utils/osi4iot_cli/installation_scripts/ufw_install.sh");
    }

    try {
        execSync(`scp ./installation_scripts/ufw_install.sh ${userName}@${nodeIP}:/home/${userName}`);
        await execShellCommand(`ssh ${userName}@${nodeIP} bash ufw_install.sh "${nodeRole}"`)
...
const installNFS = async (nodeData, org_acronym, md_hashes_array, ips_array) => {
    ...
    if (!fs.existsSync('./installation_scripts/ufw_install.sh')) {
        execSync("curl -o ./installation_scripts/ufw_install.sh https://raw.githubusercontent.com/osi4iot/osi4iot/master/utils/osi4iot_cli/installation_scripts/nfs_server_install.sh");
    }
    try {
        execSync(`scp ./installation_scripts/nfs_server_install.sh ${userName}@${nodeIP}:/home/${userName}`);
        await execShellCommand(`ssh ${userName}@${nodeIP} bash nfs_server_install.sh ${org_acronym} "${md_hashes_array}" "${ips_array}"`)
