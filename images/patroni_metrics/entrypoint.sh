...
SECRET_FILE="/run/secrets/patroni_metrics.txt"
...
if [ "$PATRONI_NUM_NODES" -le 1 ]; then
    ...
    for i in $(seq 1 "$PATRONI_NUM_NODES"); do
        PEER_NAME="patroni-metrics${i}"
        if [ "$PEER_NAME" = "$PATRONI_NAME" ]; then
            continue
        fi
        ENTRY="\"${PEER_NAME}:5011\""
        if [ -z "$ADDRS" ]; then
            ADDRS="$ENTRY"
        else
            ADDRS="$ADDRS, $ENTRY"
...
# Unlike the admin cluster, the database name here is NOT in the
# secret — post_init.sh creates it under the fixed literal name
# "iot_data_db", so it's hardcoded here to match.
export PGHOST=localhost
export PGPORT=5432
export PGUSER=patroni_metrics
export PGPASSWORD="$PATRONI_METRICS_PASSWORD"
export PGDATABASE=iot_data_db
...
echo "Starting Patroni Metrics node: $PATRONI_NAME"
echo "DCS: embedded Raft — peer ${PATRONI_NAME}:5011"
