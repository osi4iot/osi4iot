...
import IRequestWithOrganization from "../interfaces/requestWithInstitution.interface";
...
function validationMiddleware<T>(type: any, skipMissingProperties = false): express.RequestHandler {
  return async (req: IRequestWithOrganization, res, next): Promise<void> => {
    const routesWithOrganizationId = ["/organizations/organization_admin/:organizationId"];
    if (req.organization && routesWithOrganizationId.indexOf(req.route.path) !== -1) {
      req.body.institutionId = req.organization.id;
    }
    try {
