...
export const insertMLModel = async (mlModelData: IMLModel): Promise<IMLModel> => {
	const result = await pool.query(
		`INSERT INTO grafanadb.ml_model(group_id, ml_model_uid,
		...
		[mlModelData.groupId, mlModelData.mlModelUid, mlModelData.description, mlModelData.mlLibrary]
	);
	const context = {
		groupId: mlModelData.groupId,
	};
	await natsClient.jsPublish("ml_model", "create", result.rows[0].id, context);
	return result.rows[0] as IMLModel;
};

export const updateMLModelByProp = async (
	propName: string,
	...
	await pool.query(query, [mlModel.description, mlModel.mlLibrary, propValue]);
	const context = {
		groupId: mlModel.groupId,
	};
	await natsClient.jsPublish("ml_model", "update", mlModel.id, context);
};

export const deleteMLModelByProp = async (propName: string, propValue: string | number): Promise<void> => {
	const result = await pool.query(`DELETE FROM grafanadb.ml_model WHERE ${propName} = $1 RETURNING *`, [propValue]);
	await natsClient.jsPublish("ml_model", "delete", result.rows[0].id);
};
...
export const getMLModelByProp = async (propName: string, propValue: string | number): Promise<IMLModel> => {
	const response = await readQuery(
		`SELECT grafanadb.ml_model.id, grafanadb.group.org_id AS "orgId",
		...
		WHERE grafanadb.ml_model.${propName} = $1`,
		[propValue]
	);
...
export const getNumMLModels = async (): Promise<number> => {
	const result = await readQuery(`SELECT COUNT(*) FROM grafanadb.ml_model;`);
...
export const getNumMLModelsByGroupsIdArray = async (groupsIdArray: number[]): Promise<number> => {
	const result = await readQuery(
		`SELECT COUNT(*) FROM grafanadb.ml_model
		INNER JOIN grafanadb.group ON grafanadb.ml_model.group_id = grafanadb.group.id
		WHERE grafanadb.ml_model.group_id = ANY($1::bigint[])`,
		[groupsIdArray]
	);
