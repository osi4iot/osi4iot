...
export const createGroup = async (
	...
	if (isOrgDefaultGroup) {
		groupInput.groupAdminDataArray.forEach((admin) =>
			permissionsArray.push({ userId: admin.userId, permission: "Admin" })
		);
	} else {
		groupInput.groupAdminDataArray.forEach((admin) =>
			permissionsArray.push({ userId: admin.userId, permission: "Editor" })
		);
	}
	...
	const emailNotificationChannel = await grafanaApi.createNotificationChannel(
		orgKey,
		emailNotificationChannelData,
		isMainOrgDefaultGroup
	);
	...
	const telegramNotificationChannel = await grafanaApi.createNotificationChannel(
		orgKey,
		telegramNotificationChannelData,
		isMainOrgDefaultGroup
	);
	...
	if (!isMainOrgDefaultGroup) {
		await natsClient.jsPublish("group", "create", group.id);
	}
...
export const updateGroup = async (newGroupData: UpdateGroupDto, existentGroup: IGroup): Promise<void> => {
	...
	if (geoJsonDataString !== "{}") {
		...
		await natsClient.jsPublish("group", "update", existentGroup.id);
	...
	if (groupData.telegramChatId && groupData.telegramChatId !== existentGroup.telegramChatId) {
		...
		const newSettings = {
			bottoken: oldSettings.bottoken,
			uploadImage: oldSettings.uploadImage,
			chatid: groupData.telegramChatId,
		};
...
export const getGroupByWithFolderPermissionProp = async (
	propName: string,
...
export const insertGroup = async (group: IGroup): Promise<IGroup> => {
	const response = await pool.query(
		`INSERT INTO grafanadb.group (org_id, team_id, folder_id,
...
export const createView = async (group: IGroup): Promise<void> => {
	...
	await timescaledb_pool.query(
		`CREATE OR REPLACE VIEW iot_datasource.${viewName} AS SELECT timestamp, topic, payload FROM iot_data.thingData WHERE group_uid = '${group.groupUid}';`
	);
...
export const changeGroupUidByUid = async (group: IGroup): Promise<string> => {
	...
	await pool.query("UPDATE grafanadb.group SET group_uid = $1 WHERE group_uid = $2", [newGroupUid, oldGroupUid]);
...
export const updateNotificationChannelSettings = async (
	...
	const response = await pool.query(
		"UPDATE grafanadb.alert_notification SET settings = $1 WHERE id = $2 RETURNING *",
		[settings, id]
	);
	const notificationChannelId = response.rows[0].id;
	if (!isMainOrgDefaultGroup) {
		await natsClient.jsPublish("notification_channel", "update", notificationChannelId);
	}
};
...
export const deleteNotificationChannelById = async (id: number): Promise<string> => {
	...
	await natsClient.jsPublish("notification_channel", "delete", id);
	return message;
};

export const updateNotificationChannelName = async (id: number, newName: string): Promise<void> => {
	const response = await pool.query("UPDATE grafanadb.alert_notification SET name = $1 WHERE id = $2 RETURNING *", [
		newName,
		id,
	]);
	const notificationChannelId = response.rows[0].id;
...
export const getNotificationChannelById = async (id: number): Promise<INotificationChannel> => {
	const result = await pool.query(
		...
		uid, secure_settings AS "secureSettings", created, updated
	  	FROM grafanadb.alert_notification WHERE id = $1`,
		[id]
	);
...
export const getNotificationAllChannels = async (): Promise<INotificationChannel[]> => {
	const result = await pool.query(
		...
		disable_resolve_message AS "disableResolveMessage",
		uid, secure_settings AS "secureSettings", created, updated
	  	FROM grafanadb.alert_notification`
	);
	return result.rows as INotificationChannel[];
};

export const setFolderPermissionsForNewAddedMember = async (
	group: IGroup,
	groupMembersArray: CreateGroupMemberDto[]
): Promise<void> => {
	...
	if (group.folderPermission === "Edit") {
		...
		groupMembersWithEditOrAdminPrivileges = groupMembersArray.filter(
			(member) => member.roleInGroup === "Admin" || member.roleInGroup === "Editor"
		);
	}
	await giveFolderPermissionsForMemberArray(group, groupMembersWithEditOrAdminPrivileges);
};

const updateFolderPermissionsForMemberArray = async (
	group: IGroup,
	groupMembersArray: CreateGroupMemberDto[]
): Promise<void> => {
	const permissionCodes = ["None", "Viewer", "Editor", "None", "Admin"];
	...
	for (const groupMember of groupMembersArray) {
		...
		const query = pool.query(
			`UPDATE grafanadb.dashboard_acl SET permission = $1, updated = $2 WHERE dashboard_id = $3 AND user_id = $4`,
...
const giveFolderPermissionsForMemberArray = async (
	group: IGroup,
	groupMembersArray: CreateGroupMemberDto[]
): Promise<void> => {
	const permissionCodes = ["None", "Viewer", "Editor", "None", "Admin"];
	...
	for (const groupMember of groupMembersArray) {
		...
		const permission = permissionCodes.indexOf(groupMember.roleInGroup);
		const query = pool.query(
			`INSERT INTO grafanadb.dashboard_acl (org_id, dashboard_id, user_id, permission, created, updated) VALUES ($1, $2, $3, $4, $5, $6)`,
...
export const removeFolderPermissionsForMemberArray = async (
	group: IGroup,
	groupMembersArray: CreateGroupMemberDto[]
): Promise<void> => {
	const folderPermissionQueries = [];
...
const setFolderPermissions = async (
	orgId: number,
	folderId: number,
	permissionsArray: IFolderPermission[]
): Promise<void> => {
	const permissionCodes = ["None", "Viewer", "Editor", "None", "Admin"];
	const folderPermissionQueries = [];
	// tslint:disable-next-line: prefer-for-of
	for (const permissions of permissionsArray) {
		...
		if (permissions.teamId) {
			...
			const query = pool.query(
				`INSERT INTO grafanadb.dashboard_acl (org_id, dashboard_id, team_id, permission, created, updated) VALUES ($1, $2, $3, $4, $5, $6)`,
...
const updateFolderPermissionsForTeam = async (group: IGroup): Promise<void> => {
	...
	await pool.query(
		`UPDATE grafanadb.dashboard_acl SET permission = $1, updated = $2 WHERE dashboard_id = $3 AND team_id = $4`,
		[permission, now, group.folderId, group.teamId]
	);
};

export const teamMembersPermissions = async (
	teamId: number,
	groupMembersArray: (CreateGroupMemberDto | CreateGroupAdminDto)[]
): Promise<void> => {
	const permissionCodes = ["None", "Viewer", "Editor", "None", "Admin"];
...
export const haveThisUserGroupAdminPermissions = async (
	userId: number,
	teamId: number,
	orgId: number
): Promise<boolean> => {
...
export const getGroupMemberByProp = async (
	group: IGroup,
	propName: string,
	propValue: string | number
...
export const addMembersToGroup = async (
	...
	await updateNotificationChannelSettings(group.emailNotificationChannelId, newSettings, isMainOrgDefaultGroup);
...
export const udpateRoleMemberInGroup = async (
	group: IGroup,
	groupMembersArray: CreateGroupMemberDto[],
	existentGroupMemberArray: IGroupMember[]
): Promise<IMessage> => {
...
export const removeMembersInGroupsArray = async (
	groupsArray: IGroup[],
	groupMembersToRemove: IGroupMember[]
): Promise<IMessage[]> => {
	const queriesArray = [];
	...
	return await Promise.all(queriesArray).then((messages) => messages);
};
...
export const removeMembersInGroup = async (group: IGroup, groupMembersToRemove: IGroupMember[]): Promise<IMessage> => {
	if (groupMembersToRemove.length === 0)
		return { message: "All non admin members has been already removed of the group" };
	const groupMembersToRemoveFolderPermissions: CreateGroupMemberDto[] = [];
	...
	const newEmailsArray = oldAddressesArray.filter(
		(email) => memberToRemoveEmailsArray.indexOf(email) === -1 && email !== ""
	);
...
export const cleanEmailNotificationChannelForGroupsArray = async (
	userEmail: string,
	groupsArray: IGroup[]
): Promise<void> => {
	const queriesArray = [];
