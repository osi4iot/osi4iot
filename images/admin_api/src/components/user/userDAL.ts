...
export const getUserByProp = async (
	...
	const response: QueryResult = await pool.query(
		`SELECT id, name, login, email, is_admin as "isGrafanaAdmin",
					...
					FROM grafanadb.user WHERE ${propName} = $1`,
		[propValue]
	);
...
export const createOrganizationUsers = async (
	orgId: number,
	...
	usersData.forEach((user) => {
		...
		if (!user.login || user.login === "") {
			const username = `${user.firstName
				.replace(/ /g, ".")
				.toLocaleLowerCase()}.${user.surname
				.replace(/ /g, ".")
				.toLocaleLowerCase()}`;
		...
		if (!user.natsNkey) {
			user.natsNkey = "undefined";
		}
	});
	const msg_users = await grafanaApi.createUsers(usersData);
	const numCreatedUsers = msg_users.filter(
		(msg) => msg.message === "User created"
	).length;

	if (numCreatedUsers) {
		const usersModificationQuery = [];
		for (let i = 0; i < usersData.length; i++) {
			usersModificationQuery[i] = pool.query(
				"UPDATE grafanadb.user SET first_name = $1, surname = $2, nats_nkey = $3 WHERE email = $4",
				[
					usersData[i].firstName,
					usersData[i].surname,
					usersData[i].natsNkey,
					usersData[i].email,
				]
			);
		}
		...
		const usersIdArray = msg_users
			.filter((msg) => msg.message === "User created")
			.map((msg) => msg.id);
		...
		await grafanaApi.changeUsersRoleInOrganization(
			orgId,
			usersIdArray,
			usersRoleArray
		);
...
export const createFictitiousUserForService = async (
	serviceUserData: CreateUserDto
) => {
...
export const isThisUserOrgAdmin = async (
	userId: number,
	orgId: number
): Promise<boolean> => {
...
export const getOrganizationUsersForOrgIdsArray = async (
	orgIdsArray: number[]
): Promise<IUserInOrg[]> => {
	const query = `SELECT grafanadb.user.id as "userId", first_name AS "firstName", surname, login, email,
					grafanadb.org_user.org_id as "orgId", role as "roleInOrg",
...
export const getOrganizationUsersWithGrafanaAdmin = async (
	orgId: number
): Promise<IUserInOrg[]> => {
	const query = `SELECT grafanadb.user.id as "userId", first_name AS "firstName", surname,  login, email,
...
export const getOrganizationUserWithGrafanaAdminByProp = async (
	orgId: number,
	propName: string,
	propValue: string | number
): Promise<IUserInOrg> => {
...
export const getOrganizationUserByProp = async (
	orgId: number,
	propName: string,
	propValue: string | number
): Promise<IUserInOrg> => {
...
export const getOrganizationUsersByEmailArray = async (
	orgId: number,
	emailsArray: string[]
): Promise<IUserInOrg[]> => {
	const query = `SELECT grafanadb.user.id as "userId", first_name AS "firstName", surname, login, email,
...
export const updateOrganizationUser = async (
	userData: IUserInOrg | UserRegisterDto
) => {
...
export const isUsersDataCorrect = async (
	usersInputData: (CreateUserDto | CreateGlobalUserDto)[]
): Promise<boolean> => {
	usersInputData.forEach(
		(user) => (user.name = `${user.firstName} ${user.surname}`)
	);
	const namesArray = usersInputData.map((user) => user.name);
	const emailsArray = usersInputData.map((user) => user.email);
	const loginArray = usersInputData
		.map((user) => user.login)
		.filter((item) => !!item);
	if (loginArray.length !== 0 && namesArray.length !== loginArray.length) {
		throw new Error("Each user must have the same amount of data");
	}
	const forbiddenUserNames = loginArray.filter(
		(username) => username.slice(-9) === "api_admin"
	);
	...
	const response: QueryResult = await pool.query(
		`SELECT id, name, login, email FROM grafanadb.user
	...
	for (const user of existentUsers) {
		...
		if (loginArray.length !== 0) {
			...
			if (
				(sameName && sameEmail && sameLogin) !==
				(sameName || sameEmail || sameLogin)
			)
				return false;
		} else {
			if ((sameName && sameEmail) !== (sameName || sameEmail))
				return false;
		}
	}
...
export const isUserProfileDataCorrect = async (
