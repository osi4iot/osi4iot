...
export default class GrafanaApi implements IDashboardApi {
	...
	async getOrganizationByName(orgName: string): Promise<IOrganizationGrafana> {
		...
		const organization = await needle("get", url, optionsBasicAuthApiAdmin)
			.then((res) => res.body)
			.catch((err) =>
				logger.log("error", "Organization with name=${orgName} could not be found: %s", err.message)
			);
		return organization;
	}
	...
	async switchOrgContextForAdmin(newOrgId: number): Promise<IMessage> {
		...
		const message = await needle("post", url, null, optionsBasicAuthApiAdmin)
			.then((res) => res.body)
			.catch((err) =>
				logger.log("error", "Context to org. id=${newOrgId} could not be switched: %s", err.message)
			);
		return message;
	}
	...
	async getUserByLoginOrEmail(loginOrEmail: string): Promise<IUserGlobalGrafana> {
		...
		const user = await needle("get", url, optionsBasicAuthApiAdmin)
			.then((res) => res.body)
			.catch((err) =>
				logger.log("error", "Users with login or email=${loginOrEmail} could not be geted: %s", err.message)
			);
		return user;
	}
	...
	async addUsersToOrganization(orgId: number, usersData: CreateUserDto[]): Promise<IMessage[]> {
		...
		for (let i = 0; i < usersData.length; i++) {
			...
			usersAddedQueries[i] = needle("post", url, userData, optionsBasicAuthApiAdmin)
				.then((res) => res.body)
				.catch((err) => logger.log("error", "User could not be added to the organization: %s", err.message));
		}

		const msg_users = await Promise.all(usersAddedQueries).then((messages) => messages);
		return msg_users;
	}
	...
	async changeUsersRoleInOrganization(
		orgId: number,
		usersIdArray: number[],
		usersRoleArray: string[]
	): Promise<IMessage[]> {
		...
		for (let i = 0; i < usersIdArray.length; i++) {
			...
			const url = `${GrafanaApiURL}/orgs/${orgId}/users/${userId}`;
			usersCreationQueries[i] = needle("patch", url, userRole, optionsBasicAuthApiAdmin)
				.then((res) => res.body)
				.catch((err) => logger.log("error", "User role could not be changed: %s", err.message));
		}

		return await Promise.all(usersCreationQueries).then((messages) => messages);
	}
	...
	async removeUsersFromOrganization(orgId: number, usersIdArray: number[]): Promise<IMessage[]> {
		...
		for (let i = 0; i < usersIdArray.length; i++) {
			...
			usersCreationQueries[i] = needle("delete", url, null, optionsBasicAuthApiAdmin)
				.then((res) => res.body)
				.catch((err) => logger.log("error", "User could not be removed from organization: %s", err.message));
		}
	...
	async createUsers(users: CreateUserDto[]): Promise<IMessage[]> {
		...
		for (let i = 0; i < users.length; i++) {
			...
			usersCreationQueries[i] = needle("post", url, users[i], optionsBasicAuthApiAdmin)
				.then((res) => res.body)
				.catch((err) => logger.log("error", "User could not be created: %s", err.message));
		}
	...
	async deleteOrgApiAdminUser(orgId: number): Promise<IMessage> {
		...
		const user_message = await needle("get", getUserIdUrl, optionsBasicAuthApiAdmin)
			.then((res) => res.body)
			.catch((err) =>
				logger.log("error", "The user with username=${userName} could not be deleted: %s", err.message)
			);
		...
		const message = await needle("delete", url, null, optionsBasicAuthApiAdmin)
			.then((res) => res.body)
			.catch((err) =>
				logger.log("error", "The user with id=${user_message.id} could not be deleted: %s", err.message)
			);
		return message;
	}
	...
	async changeUserPassword(userId: number, newPassword: string): Promise<IMessage> {
		...
		const message = await needle("put", url, passwordData, optionsBasicAuthApiAdmin)
			.then((res) => res.body)
			.catch((err) =>
				logger.log("error", "The password to the user with id=${userId} could not be changed: %s", err.message)
			);
		return message;
	}
	...
	async getTeamsWithPaging(
		orgId: number,
		perpage: number,
		page: number,
		query?: string,
		name?: string
	): Promise<TeamsWithPaging> {
		const optionsBasicAuth = optionsBasicAuthOrgAdmin(orgId);
	...
	async getTeamMembers(orgId: number, teamId: number): Promise<TeamMember[]> {
		...
		const members = await needle("get", url, optionsBasicAuth)
			.then((res) => res.body)
			.catch((err) =>
				logger.log("error", "Members fot team with id=${teamId} could not be geted: %s", err.message)
			);
		return members;
	}
	...
	async addTeamMembers(orgId: number, teamId: number, usersId: IUserId[]): Promise<string[]> {
		...
		for (let i = 0; i < usersId.length; i++) {
			usersCreationQueries[i] = needle("post", url, usersId[i], optionsBasicAuth)
				.then((res) => {
					return res.body.message;
				})
				.catch((err) =>
					logger.log("error", "A member to the team with id=${teamId} could not be added: %s", err.message)
				);
		}
	...
	async removeMemberFromTeam(orgId: number, teamId: number, usersId: number): Promise<IMessage> {
		...
		const message = await needle("delete", url, null, optionsBasicAuth)
			...
			.catch((err) =>
				logger.log(
					"error",
					"The member with id=${usersId} could not be removed to the team with id=${teamId}: %s",
					err.message
				)
			);
		return message;
	...
	async removeMembersFromTeam(orgId: number, teamId: number, userIdsArray: number[]): Promise<IMessage[]> {
		...
		for (let i = 0; i < userIdsArray.length; i++) {
			...
			memberToRemoveQueries[i] = needle("delete", url, null, optionsBasicAuth)
				.then((res) => {
					return res.body;
				})
				.catch((err) =>
					logger.log("error", "A member to the team with id=${teamId} could not be removed: %s", err.message)
				);
		}

		return await Promise.all(memberToRemoveQueries).then((messages) => messages);
	}
	...
	async folderPermission(
		...
		const message = await needle("post", url, folderPermissionDTO, optionsToken)
			.then((res) => res.body)
			.catch((err) =>
				logger.log("error", "Permissions to folder with id=${teamId} could not be assigned: %s", err.message)
			);
		return message;
	}
	...
	async getDataSourceByName(datasourceName: string, orgKey: string): Promise<IDataSource | IMessage> {
		...
		const response = await needle("get", url, optionsToken)
			.then((res) => res.body)
			.catch((err) =>
				logger.log("error", `Data source with name: ${datasourceName} could not be obtained: %s`, err.message)
			);
		return response;
	}
	...
	async createDataSourceTimescaledb(
		...
		const message = await needle("post", url, dataSource, optionsToken)
			...
			.catch((err) =>
				logger.log(
					"error",
					`Data source for the organization with id: ${orgId} could not be created: %s`,
					err.message
				)
			);
		return message;
	...
	async updateDataSourceTimescaledb(
		...
		const message = await needle("put", url, dataSourceUpdated, optionsToken)
			.then((res) => res.body)
			.catch((err) =>
				logger.log("error", `Data source with id: ${oldDataSource.id} could not be updated: %s`, err.message)
			);
		return message;
	}
	...
	async createNotificationChannel(
		orgKey: string,
		notifChannelData: INotificationChannel,
		...
		if (!isMainOrgDefaultGroup) {
			await natsClient.jsPublish("notification_channel", "create", notificationChannel.id);
		}
		await updateNotificationChannelSettings(
			notificationChannel.id,
			notifChannelData.settings,
