...
type HostMetric struct {
	...
	// Swarm node labels — stored as JSONB
...
func collectHost(pretty bool) error {
	...
	if info.Swarm.NodeID != "" {
		// NodeInspectWithRaw works on both managers and workers for self-inspection.
		...
		if err == nil {
			...
			nodeLabels = node.Spec.Labels
		} else {
			fmt.Fprintf(os.Stderr, "[host] NodeInspect: %v (continuing)\n", err)
		}
	}
