...
--
-- Architecture: Vector's postgres sink only does plain INSERTs. To get upsert
-- semantics we expose a VIEW (host_node_state) as the sink target and put an
-- INSTEAD OF INSERT trigger on it that forwards to the real table
-- (host_node_state_data) with ON CONFLICT DO UPDATE.
--
-- A plain table rule (DO INSTEAD) causes infinite recursion in PostgreSQL
-- because the rule rewrites the INSERT before it reaches the table, including
-- the INSERT generated by the rule itself. An INSTEAD OF trigger on a view
-- does not recurse because the INSERT inside the trigger targets the
-- underlying table, not the view.

-- Real storage table — never written to directly by Vector
CREATE TABLE IF NOT EXISTS observability.host_node_state_data (
    ...
    CONSTRAINT host_node_state_data_pkey PRIMARY KEY (node_id)
);

CREATE INDEX IF NOT EXISTS idx_host_node_state_role
    ON observability.host_node_state_data (swarm_role);
...
-- INSTEAD OF INSERT trigger on the view: no recursion because the INSERT
-- inside the function targets host_node_state_data, not the view
...
INSTEAD OF INSERT ON observability.host_node_state
FOR EACH ROW EXECUTE FUNCTION observability.upsert_host_node_state();

GRANT SELECT ON observability.host_node_state TO data_source_user_org_1;
