...
import ReadMobileMotion from '../../tools/ReadingMobileMotion';
...
const ConnectionLed = styled.span.withConfig({
	shouldForwardProp: (prop) => prop !== 'isMqttConnected'
  })<ConnectionLedProps>`
...
const MobileMotionForm: FC<MobileMotionSelectFormProps> = (
    ...
    const handleSubmit = async (values: any, actions: any) => {
        if (mqttClient && !isSensorReading && mobileTopicSelected != null) {
            ...
            setGravitySensor(gravitySensor as GravitySensor);
            setAccelerationSensor(accelerationSensor as Accelerometer);
            setQuaternionSensor(quaternionSensor as AbsoluteOrientationSensor);
        }
    };
...
export default MobileMotionForm;
