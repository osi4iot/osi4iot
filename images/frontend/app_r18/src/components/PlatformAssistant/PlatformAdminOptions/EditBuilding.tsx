...
const EditBuilding: FC<EditBuildingProps> = ({ buildings, backToTable, refreshBuildings }) => {
    ...
    const {
        openFilePicker: openBuildingFileSelector,
        filesContent: buildingFileParamsFilesContent,
        loading: buildingFileParamsLoading,
        plainFiles: buildingFileParamsPlainFiles,
        clear: buildingFileParamsClear
    } = useFilePicker({
        readAs: 'Text',
        multiple: false,
        accept: '.geojson',
    ...
    useEffect(() => {
        if (
            !buildingFileParamsLoading &&
            buildingFileParamsFilesContent.length !== 0 &&
            buildingFileParamsPlainFiles.length !== 0
        ) {
            setBuildingFileLoaded(true)
            try {
                const fileContent = buildingFileParamsFilesContent[0].content;
                ...
                const buildingFileName = buildingFileParamsPlainFiles[0].name;
                setBuildingFileName(buildingFileName);
                const dateString = (buildingFileParamsPlainFiles[0] as any).lastModified;
                setBuildingFileLastModifDate(formatDateString(dateString));
                setBuildingFileLoaded(false);
                buildingFileParamsClear();
        ...
        buildingFileParamsLoading,
        buildingFileParamsFilesContent,
        buildingFileParamsPlainFiles,
        buildingFileParamsClear,
    ])
    ...
    const buildingFileButtonHandler = async () => {
        if (!buildingFileLoaded) {
            selectFile(openBuildingFileSelector, buildingFileParamsClear);
        }
    }
