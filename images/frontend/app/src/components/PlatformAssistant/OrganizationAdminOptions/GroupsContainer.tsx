...
const GroupsContainer: FC<GroupsContainerProps> = ({
    ...
    const setGroupLocationData = useCallback(
        (floorNumber: number, featureIndex: number) => {
    ...
    const showSelectSpaceOption = useCallback(() => {
        if (buildingsFiltered.length !== 0 && floorsFiltered.length !== 0) {
            ...
            const warningMessage =
                "To select an space for the group, building and floor geodata must be already entered";
        ...
        <>
            ...
            {groupsOptionToShow === GROUPS_OPTIONS.SELECT_SPACE && (
                <GroupLocationContainer
                    ...
                    setGroupLocationData={(floorNumber: number, featureIndex: number) =>
