...
const CreateAsset: FC<CreateAssetProps> = ({
    ...
    const [iconSvgString, setIconSvgString] = useState(
        storedIconSvgString !== "" ? storedIconSvgString : initialAssetType.iconSvgString,
    );
    ...
    const handleChangeOrg = (e: { value: string }, formik: FormikType) => {
        ...
        const iconSvgString = assetTypes.filter((item) => item.orgId === orgId && item.type === assetType)[0]
            .iconSvgString;
    ...
    const onSubmit = (values: any, actions: any) => {
        ...
        const assetTypeId = assetTypes.filter(
            (assetType) => assetType.orgId === orgId && assetType.type === values.assetType,
        )[0].id;
    ...
    const validationSchema = Yup.object().shape({
        ...
        longitude: Yup.number()
            .moreThan(-180, "The minimum value of longitude is -180")
            .lessThan(180, "The maximum value of longitude is 180")
            .required("Required"),
        latitude: Yup.number()
            .moreThan(-90, "The minimum value of latitude is -90")
            .lessThan(90, "The maximum value of latitude is 90")
            .required("Required"),
        iconRadio: Yup.number()
            .min(0.2, "The minimum value of the icon ratio is 0.2m")
            .max(2, "The maximum value of the icon ratio is 2m")
            .required("Required"),
        iconSizeFactor: Yup.number()
            .min(0.1, "The minimum value of the icon size factor is 0.1")
            .max(2, "The maximum value of the icon size factor is 2")
            .required("Required"),
    });
    ...
    const selectLocation = (initialAssetData: InitialAssetData) => {
        ...
        if (!group) {
            const warningMessage =
                "The groupId indicated not exists or not corresponds to a group managed by the user.";
                        ...
                        <Form>
                            <ControlsContainer>
                                ...
                                <AssetLocationContainerDiv>
                                    <FormikControl control="input" label="Longitude" name="longitude" type="text" />
                                    <FormikControl control="input" label="Latitude" name="latitude" type="text" />
                                    <FormikControl control="input" label="Icon ratio" name="iconRadio" type="text" />
                                    <FormikControl
                                        control="input"
                                    ...
                                    <SelectAssetLocationDivButtonContainer>
                                        <SelectLocationButton
                                            type="button"
                                            onClick={() => selectLocation(formik.values)}
                                        >
                                            Select location
                                        </SelectLocationButton>
                                    </SelectAssetLocationDivButtonContainer>
                                ...
                                <CodeMirrorWrapper fontSize="14px">
                                    <label>Topic references</label>
                                    <div className="cm-wrapper">
                                        <CodeMirror
                                            value={formik.values.topicsRef}
                                            height="300px"
                                            theme={oneDark}
                                            extensions={[
                                                json(),
                                            ...
                                            onChange={(value) => {
                                                formik.setFieldValue("topicsRef", value);
                                            }}
                                            onBlur={() => {
                                                formik.setFieldTouched("topicsRef", true);
                                            }}
                                            basicSetup={{
                                                lineNumbers: true,
                                                foldGutter: true,
                                    ...
                                    <FieldErrorScroller name="topicsRef" />
                                </CodeMirrorWrapper>
                                <CodeMirrorWrapper fontSize="14px">
                                    <label>Sensor references</label>
                                    <div className="cm-wrapper">
                                        <CodeMirror
                                            value={formik.values.sensorsRef}
                                            height="300px"
                                            theme={oneDark}
                                            extensions={[
                                                json(),
                                            ...
                                            onChange={(value) => {
                                                formik.setFieldValue("sensorsRef", value);
                                            }}
                                            onBlur={() => {
                                                formik.setFieldTouched("sensorsRef", true);
                                            }}
                                            basicSetup={{
                                                lineNumbers: true,
                                                foldGutter: true,
                                    ...
                                    <FieldErrorScroller name="sensorsRef" />
                                </CodeMirrorWrapper>
                            </ControlsContainer>
                            <FormButtonsProps
                                onCancel={onCancel}
